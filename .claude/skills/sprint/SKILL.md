...
- `references/run.md` — **sprint execution** (the default model: phase-scripted daemon sessions driven off the monitor stream)
...
- `references/investigations.md` — nerd-snipe gate before impl (flakies, recurring bugs, perf/security findings); load-bearing spawn shape (`mcx claude spawn`, NOT the Agent tool — #2009)
...
- **Execute via `mcx claude spawn` sessions, 2–3 concurrent lanes max**
  (`references/run.md`) — capacity comes from turnover, not headcount.
- **Never implement directly.** Always delegate to a spawned session.
  (Meta-file fixes via `plan.md` Step 1a are the one orchestrator-direct
  exception, by design.)
...
- **No session sits idle-hot.** A session with nothing to do gets `bye`d or
  its resume point written to the ledger. Idle context re-fed across a
  cache expiry is the single largest cost class we have measured (sprint 78).
...
- **Wait on the monitor stream, never sleep** — and never `mcx claude wait`
  polling (legacy interactive CLI; loses event enrichment — see run.md).
