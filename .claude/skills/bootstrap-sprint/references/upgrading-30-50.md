...
| 26 | **Repo type affects the merge model.** `gh api users/<owner> --jq .type` — `Organization` repos get native merge queue (Team/Enterprise tier); `User` repos do not, at any tier. User-owned repos need a long-lived "mergemaster" sonnet session that runs `gh pr update-branch --rebase` + polls CI + lets `mcx pr merge --auto` fire as PRs hit `qa:pass`. Document the trade-off in `run.md` so the next operator understands. | `discovery.md`, `design.md` (merge gate) |
