...
- **Two TypeScript packages, on purpose**: `typescript` is 7.x (the native Go compiler) and backs `bun typecheck` / `tsc -b`. `typescript-5` is an npm alias for `typescript@5.9.3` and backs everything that parses source as an AST — `scripts/rules/**` and `.claude/skills/estimate/**`. TS7 ships no standalone parser (`createSourceFile`/`forEachChild` are gone; the `unstable/*` API needs a Project), and the rule engine is deliberately program-free. `import ts from "typescript-5"` in those trees is correct — do not "fix" it to `"typescript"`.
