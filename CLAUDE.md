...
- **Duplication & abstraction**: this codebase tolerates duplication on purpose — DRY optimizes for human constraints that mostly don't bind a Claude maintainer. **Abstract the nouns (data contracts), duplicate the verbs (behavior); abstract what changes together, duplicate what changes independently.** A rule must mechanize an *invariant*, never enforce DRY for its own sake — never write a rule that forces de-duplication of independently-changing verbs. See `docs/architecture/duplication.md`.
