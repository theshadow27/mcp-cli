 ...
 * and server-initiated request handling (approval flow).
 */
...
export interface RpcClientOptions {
  ...
  /** Called for server-initiated requests (has id + method, e.g. approval requests). */
...
export class CodexRpcClient {
  ...
  private readonly onNotification: RpcClientOptions["onNotification"];
  private readonly onServerRequest: RpcClientOptions["onServerRequest"];

  constructor(proc: CodexProcess, opts: RpcClientOptions = {}) {
    this.proc = proc;
  ...
  /** Route an incoming message from the process. Call this from CodexProcess.onMessage. */
