...
/** Clean env for git commands — strip GIT_DIR/GIT_WORK_TREE so tests don't inherit parent repo. */
/** Keys that git uses to override repo discovery — must be unset for isolated test repos. */
const GIT_ENV_KEYS = ["GIT_DIR", "GIT_WORK_TREE", "GIT_INDEX_FILE"] as const;

/** Unset git env vars so child git processes discover the test repo, not the parent. */
function clearGitEnv(): Record<string, string | undefined> {
...
/** Restore previously saved git env vars. */
...
function setupRepo(repoDir: string): { cache: CloneCache; scope: ResolvedScope } {
  ...
  // Initialize git repo — env vars already cleaned in beforeEach
...
beforeEach(() => {
  ...
  savedGitEnv = clearGitEnv();
});

afterEach(() => {
  restoreGitEnv(savedGitEnv);
...
describe("pull", () => {
  describe("full sync", () => {
    ...
    test("updates files when remote version is newer", async () => {
      ...
      execSync("git add -A && git commit -m 'existing'", { cwd: repoDir, stdio: "pipe" });
