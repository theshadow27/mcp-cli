...
/** Build env for git commands, stripping vars that override repo discovery (e.g. from pre-commit hooks). */
function gitEnv(): Record<string, string> {
  const env: Record<string, string> = {};
  for (const [k, v] of Object.entries(process.env)) {
    if (v != null && k !== "GIT_DIR" && k !== "GIT_WORK_TREE" && k !== "GIT_INDEX_FILE") {
      env[k] = v;
    }
  }
  return env;
...
export async function pull(opts: PullOptions): Promise<PullResult> {
  ...
  try {
    ...
    if (totalChanges > 0) {
      const gitOpts = { cwd: repoDir, stdio: "pipe" as const, env: gitEnv() };
