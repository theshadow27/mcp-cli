...
registerHelp("claude auth", {
  ...
  notes: [
    ...
    "active) from the on-disk 5h/7d snapshots: stay on CURRENT until oauth is",
    "expiring or remaining 5h < 8% or remaining 7d < 12%, else smallest eligible",
    "plan, with a harvest hop for a 5h window that still has >30% and resets in",
    "<45m. A profile with no stored credentials, no usable token, or no trustworthy",
    "numbers is never recommended; when nothing qualifies `ls` prints `> (none)` and no",
    "row is marked. `load --auto` switches to that `>` row only when it differs.",
  ],
  usage: [
