...
registerHelp("claude spawn", {
  ...
  options: [
    ...
    ["--no-profile", "Run on the bare daemon env, ignoring the default-profile config"],
  ],
  examples: [
...
registerHelp("claude bye", {
  ...
  usage: [
    'mcx claude bye <session> "wrap up"',
    "mcx claude bye --all [-d <domain>]",
    "mcx claude bye --all-domains",
    "mcx claude bye <session> --clean",
  ],
  options: [
    ["--clean", "Also remove the worktree and delete its branch"],
    ["--keep, --keep-worktree", "No-op: keeping the worktree is the default"],
    ["--all, -a", "End every session in scope (this domain, or -d <domain>)"],
    ["-d, --domain <name>", "Scope --all to a named domain"],
    ["--all-domains", "End EVERY session in EVERY domain on this machine"],
  ],
});
...
registerHelp("claude auth", {
  ...
  notes: [
    ...
    "recorded when ownership cannot be proven. Neither flag switches the live identity.",
    "",
    "`ls` marks the sticky recommended identity with `>` (and `*>` when it is also",
    "active) from the on-disk 5h/7d snapshots: stay on CURRENT until remaining 5h",
    "< 8% or remaining 7d < 12%, else smallest eligible plan, with a harvest hop for",
    "a 5h window that still has >30% and resets in <45m. Access-token expiry is",
    "informational — `load` writes the stored refresh token and Claude Code does the",
    "oauth exchange. A profile with no stored credentials or no trustworthy quota",
    "numbers is never recommended; when nothing qualifies `ls` prints `> (none)` and",
    "no row is marked. `load --auto` switches to that `>` row only when it differs.",
    "Rows default to soonest 7d reset, then least 7d remaining (`--sort name` for A–Z).",
    "A 5H/7D percent whose reset is already in the past prints `--` (the cached number",
    "is from the previous window) and counts as 0 remaining. Future EXPIRES/5H-RESET/",
    "7D-RESET stamps get a relative `(3d)` / `(4h)` / `(15m)`.",
    "",
    "Each successful quota stamp appends a 5h/7d sample onto `quota.history` (ring of",
    "48). `ls` projects 5H-ETA / 7D-ETA from that series: `ok` if the slope will not",
    "empty the window before reset (or is idle), `40m miss` if it will, `-` until two",
    "samples in the current window span at least a minute. `--fetch` is not required",
    "to *see* the numbers — it only refreshes them. The picker / `load --auto` still",
    "use remaining percent, not pace; autoload-by-shortfall comes after this lands.",
    "",
    "`refresh <profile>` posts a refresh_token grant for a *parked* oauth profile and",
    "writes the new tokens onto that profile file only. It never touches the live",
  ...
  usage: [
    ...
    "mcx claude auth load --auto [--json]",
    "mcx claude auth ls [--json] [--sort 7d-reset|name]",
  ...
  options: [
    ...
    ["--fetch-all", "Re-query every unexpired profile one at a time (429s back off; bounded by a 2m budget)"],
    ["--sort <mode>", "ls row order: 7d-reset (default, soonest then least remaining) or name"],
...
registerHelp("claude profile", {
  ...
  notes: [
    ...
    "  --profile <name>  >  default-profile config  >  none",
    ...
    "",
    "A repo .mcx.yaml is DESELECT-ONLY: `profile: null` opts the repo out, but a named",
    "`profile: <name>` is ignored with a warning. A .mcx.yaml arrives by `git clone`, and",
    "letting repo content pick credentials for an auto-approving agent is not a setting.",
    "",
    ...
    "",
    "A profile may only set AWS_*, ANTHROPIC_*, BEDROCK_*, CLAUDE_CODE_*, VERTEX_* and a",
    "few named Vertex keys. PATH, NODE_OPTIONS, LD_PRELOAD, HOME, GIT_SSH_COMMAND and",
    "MCP_CLI_DIR are code execution and daemon reattachment, not configuration.",
    "A bare `KEY` line (no `=`) UNSETS that variable in the child, so a Bedrock profile",
    "can drop an ANTHROPIC_API_KEY the daemon inherited instead of running on both.",
  ],
  usage: [
