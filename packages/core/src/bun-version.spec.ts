...
describe("assertBunVersion", () => {
  ...
  it("does not exit when minimum is a known-older Bun release", () => {
    ...
    try {
      // Any currently-shipping Bun (>=1.4.0) should satisfy a >=1.2.0 requirement
  ...
  it("exits when injected version is one patch below minimum", () => {
    ...
    try {
      expect(() => assertBunVersion("1.4.1", "1.4.0")).toThrow("process.exit called");
      expect(exitSpy).toHaveBeenCalledWith(1);
    } finally {
      exitSpy.mockRestore();
      stderrSpy.mockRestore();
  ...
  it("does not exit for a canary build at the exact minimum version", () => {
    ...
    try {
      expect(() => assertBunVersion("1.4.0", "1.4.0-canary.20250401")).not.toThrow();
      expect(stderrSpy).not.toHaveBeenCalled();
    } finally {
      exitSpy.mockRestore();
      stderrSpy.mockRestore();
  ...
  it("exits for a canary build below the minimum version", () => {
    ...
    try {
      expect(() => assertBunVersion("1.4.1", "1.4.0-canary.20250401")).toThrow("process.exit called");
      expect(exitSpy).toHaveBeenCalledWith(1);
    } finally {
      exitSpy.mockRestore();
      stderrSpy.mockRestore();
