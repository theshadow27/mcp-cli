...
// ── Kiro host-auth callback (`_kiro/auth/getAccessToken`) ──
//
...
// The `kiro-auth-callback` fake mode reproduces exactly that. These tests guard the
// bug where the client answered the callback with `{}` and every kiro turn died at
...
describe("AcpSession (kiro host-auth callback)", () => {
  ...
  test("turn completes when KIRO_API_KEY is set (token handed to the auth callback)", async () => {
    const { session } = makeSession({
      ...
      env: { KIRO_API_KEY: "test-access-token" },
    });
