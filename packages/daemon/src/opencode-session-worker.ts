/**
 * Bun Worker hosting OpenCode agent session management via MCP Server.
 *
 * Manages OpenCodeSession instances, each spawning an `opencode serve`
 * process and communicating via HTTP REST + SSE.
 *
 * Mirrors acp-session-worker.ts but for the OpenCode protocol.
 *
 * Protocol:
...
async function handlePrompt(args: Record<string, unknown>): Promise<{
  ...
  if (sessionId) {
    ...
    const sid = sessionId;
    const session = new OpenCodeSession(sid, config, (event) => forwardSessionEvent(sid, event));
    ...
    self.postMessage({
      type: "metrics:inc",
      name: "opencode_process_spawn_total",
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : String(err);
      return { content: [{ type: "text", text: `Failed to start OpenCode session: ${message}` }], isError: true };
    }
  }
  ...
  self.postMessage({
    ...
    name: "opencode_turn_duration_seconds",
    value: durationS,
  });
...
async function startServer(): Promise<void> {
  mcpServer = new Server({ name: OPENCODE_SERVER_NAME, version: "0.1.0" }, { capabilities: { tools: {} } });

  mcpServer.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: OPENCODE_TOOLS.map((t) => ({
      name: t.name,
